// ============================================================================
// VillageSquare Meet — meeting engine
//
// This is the original vanilla `js/app.js` ported to a TypeScript module. The
// React layer (src/App.tsx + src/components/*) renders the exact markup from the
// old index.html ONCE; this module then drives that DOM imperatively, exactly
// like the original did. The grid layout solver and the <video>/<audio> tile
// diffing are preserved verbatim on purpose — they exist to avoid flicker and
// mis-attached tracks, which a per-tile React reconciler would reintroduce.
//
// FOUR WebRTC audio fixes were ADDED on top of the original (search "AUDIO FIX"):
//   a. autoplay gating via RoomEvent.AudioPlaybackStatusChanged + room.startAudio()
//   b. background/restore via visibilitychange (Chrome suspends audio)
//   c. reconnect via RoomEvent.Reconnected (re-attach + startAudio)
//   d. clean teardown on disconnect (stop local mediaStreamTracks)
// ============================================================================

import * as LK from 'livekit-client';

// LiveKit objects (Room, participants, publications) are used loosely here, the
// same way the original treated `window.LivekitClient` values — typing them as
// `any` keeps the ported imperative code faithful instead of fighting the SDK.
/* eslint-disable @typescript-eslint/no-explicit-any */

// Small typed-`any` DOM accessor so the ported getElementById logic is unchanged.
...
let room: any = null, myRole = 'guest', roomId = '', livekitUrl = '', shareUrl = '', livestreamUuid = '';
let camOn = true, micOn = true, sharing = false, handRaised = false;
let curPanel: string | null = null, unread = 0;
let pinned = '';            // identity spotlighted for everyone ('' = none)
const chatLog: any[] = []; const handsUp = new Set<string>(); const speaking = new Set<string>();
let pjStream: any = null, pjCamOn = true, pjMicOn = true, pendingRoom = '';
...
function hashOf(s: any) { s = String(s || '?'); let h = 0x811c9dc5; for (let i = 0; i < s.length; i++) { h ^= s.charCodeAt(i); h = Math.imul(h, 0x01000193) >>> 0; } return h >>> 0; }
...
// ---------- helpers ----------
export function show(id: string) { document.querySelectorAll('.screen').forEach(s => s.classList.remove('active')); $(id).classList.add('active'); }
function initials(n: any) { n = (n || '').trim(); if (!n) return '?'; const p = n.split(/\s+/); return (p[0][0] + (p[1] ? p[1][0] : '')).toUpperCase(); }
function esc(s: any) { return (s || '').replace(/[&<>"]/g, (c: string) => (({ '&': '&amp;', '<': '&lt;', '>': '&gt;', '"': '&quot;' } as any)[c])); }
function clock() { return new Date().toLocaleTimeString([], { hour: 'numeric', minute: '2-digit' }); }
let _toastTimer: any;
function toast(m: string, ms = 2600) { const t = $('toast'); t.textContent = m; t.classList.add('show'); clearTimeout(_toastTimer); _toastTimer = setTimeout(() => t.classList.remove('show'), ms); }
function setClock() { const el = $('lpClock'); if (el) el.textContent = clock() + ' · ' + new Date().toLocaleDateString([], { weekday: 'short', month: 'short', day: 'numeric' }); }

async function apiFetch(path: string, { method = 'GET', body, auth = true, multipart = false }: any = {}) {
  const headers: any = {}; if (auth && accessToken) headers['Authorization'] = 'Bearer ' + accessToken;
  let payload: any; if (multipart) { payload = body; } else if (body) { headers['Content-Type'] = 'application/json'; payload = JSON.stringify(body); }
  const resp = await fetch(apiBase() + path, { method, headers, body: payload });
  let json: any = {}; try { json = await resp.json(); } catch {}
...
function rememberHostMeeting(m: any) { try { localStorage.setItem(HOST_MEETING_KEY, JSON.stringify(m)); } catch {} }
...
async function refreshRejoinCard() {
  const card = $('rejoinCard'); if (!card) return;
  ...
  $('rejoinRoom').textContent = m.title ? `${m.title} · ${m.roomId}` : m.roomId;
  card.style.display = 'flex';
}

export async function rejoinAsHost() {
  ...
  if (!accessToken) { pendingRejoin = true; show('hostSignin'); toast('Sign in to rejoin as host'); setTimeout(() => $('hEmail').focus(), 50); return; }
  const btn: any = document.querySelector('#rejoinCard .rj-btn'); if (btn) { btn.disabled = true; btn.textContent = 'Rejoining…'; }
  let seed: any = null;
  try {
    ...
    if (seed) { try { seed.getTracks().forEach((t: any) => t.stop()); } catch {} }
...
// ---------- landing ----------
export function onCode() { $('btnJoin').disabled = !$('joinCode').value.trim(); }
export function goHostSignin() { if (accessToken) { openHostSetup(); } else { show('hostSignin'); setTimeout(() => $('hEmail').focus(), 50); } }
...
function openHostSetup() {
  ...
  $('hsAv').textContent = initials(myName() || 'Host');
  updatePjBtns(); show('hostSetup'); startPreview();
  setTimeout(() => $('mTitle').select(), 50);
}
export function cancelHostSetup() { stopPreview(); show('landing'); }
export function goGuest() { const v = $('joinCode').value.trim(); if (!v) return; checkAndProceed(normalizeRoom(v)); }
function normalizeRoom(v: string) { try { if (v.startsWith('http')) { const u = new URL(v); return u.searchParams.get('room') || v; } } catch {} return v.trim(); }

// Gate on live status before asking for a name. Not live → branded notice.
let pendingTitle = '';
async function checkAndProceed(rawRoom: string) {
  const roomIn = (rawRoom || '').trim(); if (!roomIn) return;
  pendingRoom = roomIn; pendingTitle = '';
  const btn = $('btnJoin'); const orig = btn ? btn.textContent : ''; if (btn) { btn.disabled = true; btn.textContent = 'Checking…'; }
  try {
  ...
  } catch (e: any) { console.warn('meeting-status check unavailable, proceeding to join:', e.message); openPrejoin(); }
  finally { if (btn) { btn.disabled = false; btn.textContent = orig || 'Join'; } }
}
function showNotLive(room: string) { $('nlRoom').textContent = room; show('notlive'); }
export function checkAgain() { const b = $('btnCheckAgain'); b.disabled = true; b.innerHTML = '<span class="material-symbols-rounded">progress_activity</span> Checking…'; checkAndProceed(pendingRoom).finally(() => { b.disabled = false; b.innerHTML = '<span class="material-symbols-rounded">refresh</span> Check again'; }); }

// ---------- host: login ----------
export async function doLogin() {
  const email = $('hEmail').value.trim(), pw = $('hPass').value;
  ...
  const btn = $('btnLogin'); btn.disabled = true; btn.innerHTML = '<span class="material-symbols-rounded">progress_activity</span> Signing in…';
  try {
    ...
    $('lpAvatar').textContent = initials(email);
    if (pendingRejoin) { pendingRejoin = false; show('landing'); await rejoinAsHost(); return; }
    openHostSetup();
  } catch (e: any) { err.textContent = e.message; err.classList.add('show'); }
  finally { btn.disabled = false; btn.innerHTML = '<span class="material-symbols-rounded">login</span> Sign in'; }
}

// ---------- host: start meeting ----------
export async function startMeeting() {
  const title = $('mTitle').value.trim() || 'VillageSquare Meeting';
  const err = $('startErr'); err.classList.remove('show');
  const btn = $('btnStart'); btn.disabled = true; btn.innerHTML = '<span class="material-symbols-rounded">progress_activity</span> Starting…';
...
function myName() { return $('hEmail')?.value?.split('@')[0] || ''; }

// ---------- guest: pre-join ----------
function openPrejoin() {
  previewCtx = 'pj';
  $('pjRoom').textContent = pendingRoom;
  $('pjTitle').textContent = pendingTitle ? ('Join “' + pendingTitle + '”') : 'Join the meeting';
  $('pjName').value = localStorage.getItem('vsm_name') || '';
...
function pvEl(part: string) {
  const ids: any = previewCtx === 'hs'
...
function stopPreview() { if (pjStream) { pjStream.getTracks().forEach((t: any) => t.stop()); pjStream = null; } detachPreview(); }
...
function applyPj() {
  if (pjStream) {
    pjStream.getVideoTracks().forEach((t: any) => t.enabled = pjCamOn);
    pjStream.getAudioTracks().forEach((t: any) => t.enabled = pjMicOn);
  }
...
export function pjMic() { pjMicOn = !pjMicOn; applyPj(); }
...
export async function pjCam() {
  if (_pjCamBusy) return;
  _pjCamBusy = true;
  const btn = pvEl('cam'); if (btn) btn.disabled = true;
  try {
    if (pjCamOn) {
      if (pjStream) pjStream.getVideoTracks().forEach((t: any) => { try { t.stop(); } catch {} pjStream.removeTrack(t); });
      pjCamOn = false;
...
export function onPjName() { const v = $('pjName').value.trim(); $('btnGuestJoin').disabled = !v; updatePjTag(); }
function updatePjTag() { const v = $('pjName').value.trim() || 'You'; $('pjTag').textContent = v; $('pjAv').textContent = initials(v); }
export function leavePrejoin() { stopPreview(); show('landing'); }

// ---------- guest: join ----------
export async function guestJoin() {
  const name = $('pjName').value.trim(); if (!name) return;
  localStorage.setItem('vsm_name', name);
  camOn = pjCamOn; micOn = pjMicOn; myRole = 'guest';
  const btn = $('btnGuestJoin'); btn.disabled = true; btn.innerHTML = '<span class="material-symbols-rounded">progress_activity</span> Joining…';
  let seed: any = null;
  try {
    ...
    if (seed) { try { seed.getTracks().forEach((t: any) => t.stop()); } catch {} seed = null; }
...
function chime(kind: string) {
  try {
    _actx = _actx || new ((window as any).AudioContext || (window as any).webkitAudioContext)();
...
let _noiseBuf: any = null;
function _noiseBuffer(ac: any) {
...
function _tone(ac: any, out: any, { freq, at = 0, dur = .2, type = 'sine', peak = 1, glide = null as any, attack = .012 }: any) {
...
function _noise(ac: any, out: any, { at = 0, dur = .12, peak = 1, f0 = 2000, sweep = null as any, q = 1, type = 'bandpass' }: any) {
  const t = ac.currentTime + at;
...
const REACTION_SOUNDS: any = {
  '\u{1F44D}': (ac: any, o: any) => { _tone(ac, o, { freq: 1046, dur: .11, peak: .9 }); _tone(ac, o, { freq: 1568, at: .08, dur: .16, peak: .8 }); },              // approving ping
  '❤️': (ac: any, o: any) => { _tone(ac, o, { freq: 110, dur: .16, type: 'sine', peak: 1, attack: .006 });                                        // lub
                       _tone(ac, o, { freq: 92, at: .19, dur: .22, type: 'sine', peak: .85, attack: .006 }); },                                   // dub
  '\u{1F389}': (ac: any, o: any) => { [523, 659, 784, 1046].forEach((f, i) => _tone(ac, o, { freq: f, at: i * .055, dur: .2, type: 'triangle', peak: .7 }));         // party fanfare
                    _noise(ac, o, { at: .2, dur: .35, peak: 1.1, f0: 5000, sweep: 1200, type: 'highpass' }); },                                   // confetti hiss
  '\u{1F44F}': (ac: any, o: any) => { [0, .11, .21, .3].forEach((t, i) => _noise(ac, o, { at: t, dur: .075, peak: i ? 2.0 : 2.7, f0: 1600 + Math.random() * 900, q: .7 })); }, // applause
  '\u{1F602}': (ac: any, o: any) => { [0, .13, .25, .36].forEach((t, i) => _tone(ac, o, { freq: 520 - i * 45, at: t, dur: .11, type: 'triangle', peak: .85, glide: 400 - i * 40 })); }, // ha-ha-ha
  '\u{1F62E}': (ac: any, o: any) => { _tone(ac, o, { freq: 330, dur: .42, type: 'sine', peak: .8, glide: 990 }); },                                          // rising gasp
  '\u{1F525}': (ac: any, o: any) => { _noise(ac, o, { dur: .5, peak: 2.2, f0: 3400, sweep: 420, type: 'lowpass', q: .8 });                                      // whoosh
                    [0, .09, .19, .29].forEach(t => _noise(ac, o, { at: t, dur: .05, peak: .8, f0: 2600 + Math.random() * 1800, q: 2 })); },            // crackle
  '\u{1F64C}': (ac: any, o: any) => { [659, 880, 1319].forEach((f, i) => _tone(ac, o, { freq: f, at: i * .07, dur: .28, type: 'triangle', peak: .75 })); },         // uplifting triad
};
...
const REACTION_SAMPLES: any = { '\u{1F44F}': 'sounds/clap.mp3', '\u{1F602}': 'sounds/laugh.mp3' };
const _sampleBufs: any = {};
function _loadSample(ac: any, url: string) {
  if (!_sampleBufs[url]) {
...
function warmReactionSamples() {
  try { const ac = _ac(); Object.values(REACTION_SAMPLES).forEach((u: any) => _loadSample(ac, u).catch(() => {})); } catch {}
}

// A burst of reactions shouldn't turn into noise soup.
let _sndTimes: number[] = [];
function reactionSound(emoji: string) {
  ...
  try {
    ...
    if (url) {
      _loadSample(ac, url).then((buf: any) => {
...
async function publishLocalMedia(seed: any) {
  ...
  try {
    ...
    if (at || vt) {
      try {
        ...
        try { seed.getTracks().forEach((t: any) => t.stop()); } catch {}
      }
    }
...
async function connectRoom(token: string, displayName: string, title: string, seedStream: any) {
  ...
  ['ParticipantConnected', 'ParticipantDisconnected', 'TrackSubscribed', 'TrackUnsubscribed', 'TrackPublished', 'TrackUnpublished', 'TrackMuted', 'TrackUnmuted', 'LocalTrackPublished', 'LocalTrackUnpublished']
    .forEach(ev => room.on((LK.RoomEvent as any)[ev], () => { syncPresentations(); renderGrid(); renderPeople(); }));
  ...
  ['TrackMuted', 'TrackUnmuted', 'LocalTrackPublished', 'LocalTrackUnpublished']
    .forEach(ev => room.on((LK.RoomEvent as any)[ev], (_a: any, p: any) => { if (!p || p === room.localParticipant) syncLocalMediaState(); }));
  room.on(LK.RoomEvent.ParticipantConnected, (p: any) => {
    chime('join'); toast((p.name || p.identity) + ' joined');
  ...
  room.on(LK.RoomEvent.ParticipantDisconnected, (p: any) => {
    chime('leave'); handsUp.delete(p.identity); speaking.delete(p.identity);
  ...
  room.on(LK.RoomEvent.ActiveSpeakersChanged, (s: any) => { speaking.clear(); s.forEach((p: any) => speaking.add(p.identity)); updateSpeaking(); });
  room.on(LK.RoomEvent.Disconnected, () => cleanup());
  room.on(LK.RoomEvent.DataReceived, (payload: any, p: any) => { try { handleData(JSON.parse(new TextDecoder().decode(payload)), p); } catch {} });

  // ----- AUDIO FIX (a): autoplay gating -----
  // If the browser blocks audio playback (autoplay policy), remote audio is
  // silent until a user gesture. Surface a recoverable "Tap to enable sound"
  // affordance and clear it once playback is allowed.
  room.on(LK.RoomEvent.AudioPlaybackStatusChanged, () => updateAudioGate());

  // ----- AUDIO FIX (c): reconnect -----
  // A network glitch can leave <audio>/<video> detached and audio dead. On a
  // successful reconnect, re-run the grid render (re-attaches every remote
  // track) and re-arm audio playback.
  room.on(LK.RoomEvent.Reconnecting, () => toast('Reconnecting…'));
  ...
  $('mtTitle').textContent = title || 'Meeting';
  $('mtCode').textContent = roomId;
  show('meeting'); renderGrid(); renderPeople(); updateDock(); warmReactionSamples();
  updateAudioGate();
  toast('You joined the meeting');
}

// ----- AUDIO FIX (a): the gate affordance -----
function updateAudioGate() {
  const g = $('audioGate'); if (!g) return;
  const blocked = !!room && room.canPlaybackAudio === false;
  g.classList.toggle('show', blocked);
}
export async function enableAudio() {
  if (!room) return;
  try { await room.startAudio(); } catch (e) { console.warn('startAudio failed', e); }
  try { _actx && _actx.resume && _actx.resume(); } catch {}
  updateAudioGate();
}

function handleData(d: any, p: any) {
  const who = p ? (p.name || p.identity) : 'Someone';
...
// ---------- grid ----------
function parts(): any[] { return room ? [room.localParticipant, ...room.remoteParticipants.values()] : []; }
...
function targetAspect(container: any, n: number, specs: any) {
...
function solveLayout(n: number, container: any, gap: number, ar: number, maxAR: number, minCols: number, fill: boolean): any {
  let best: any = null;
...
function scrollLayout(n: number, container: any, gap: number, ar: number): any {
...
const tileEls = new Map<string, any>();
let stripEl: any = null;
...
function isScreenKey(k: any) { return typeof k === 'string' && k.endsWith(SCREEN_SUFFIX); }
function keyIdentity(k: string) { return isScreenKey(k) ? k.slice(0, -SCREEN_SUFFIX.length) : k; }
...
function updateTile(el: any, spec: any, { local, isPinned, grid }: any) {
  ...
  if (isPinned && v && vsel) {
    const setAR = (w: number, h: number) => { if (w && h) { el.style.setProperty('--pin-ar', w + '/' + h); grid.classList.toggle('pin-portrait', h > w); } };
    const dim = vsel.pub.dimensions;
    if (dim) setAR(dim.width, dim.height);
    if (!v.dataset.arWired) {
      v.dataset.arWired = '1';
  ...
  if (wantA !== (el.dataset.asid || '')) {
    ...
    if (wantA) { const a: any = mic.track.attach(); a.style.display = 'none'; el.appendChild(a); }
  ...
  if (isPinned && !badge) {
    badge = document.createElement('div'); badge.className = 'tpin';
  ...
  if (myRole === 'host' && !isScreen && !more) {
    ...
    more.onclick = (ev: any) => { ev.stopPropagation(); openTileMenu(el.dataset.id, el.dataset.nm || el.dataset.id, el.classList.contains('local'), ev.currentTarget); };
    el.appendChild(more);
...
function placeChildren(container: any, els: any[]) {
...
function renderGrid() {
  if (!room) return; const grid = $('grid');
  const ps = renderables();
  const phone = isPhonePortrait();
  const pinIdx = pinned ? ps.findIndex((t: any) => t.key === pinned) : -1;
  const hasPin = pinIdx >= 0;
  if (hasPin) ps.unshift(ps.splice(pinIdx, 1)[0]);
  const n = ps.length;
  let c: any, centred = false, rowCount = 1, fit = false;
  ...
  const mainEls: any[] = [], stripTiles: any[] = [];
  ps.forEach((spec: any, i: number) => {
  ...
  const alive = new Set(ps.map((t: any) => t.key));
  tileEls.forEach((el, id) => {
    if (alive.has(id)) return;
    el.querySelectorAll('video,audio').forEach((m: any) => { try { m.srcObject = null; } catch {} });
    el.remove(); tileEls.delete(id);
  });
  $('peopleCnt').textContent = parts().length;
}
...
function updateSpeaking() { document.querySelectorAll<HTMLElement>('.tile').forEach(t => t.classList.toggle('speaking', speaking.has(t.dataset.id as string))); }
...
function setPin(identity: string) {
...
async function moderate(identity: string, action: string, label: string) {
  try {
    await apiFetch('/livestreams/' + encodeURIComponent(roomId) + '/moderate', { method: 'POST', body: { identity, action } });
    toast(label);
  } catch (e: any) { toast('Could not ' + action.replace('_', ' ') + ': ' + e.message, 4000); }
}

let tileMenuFor = '';
function openTileMenu(identity: string, name: string, isLocal: boolean, anchor: any) {
  const m = $('tileMenu'); if (!m) return;
...
function closeTileMenu() { const m = $('tileMenu'); if (m) { m.classList.remove('open'); } tileMenuFor = ''; }
...
function deviceError(e: any, kind: string, on: boolean) {
  const n = (e && e.name) || '';
...
// ---------- reactions ----------
export function toggleReact() { $('reactBar').classList.toggle('open'); $('dReact').classList.toggle('accent'); }
export function react(e: string) { const me = room?.localParticipant?.name || 'You'; floatEmoji(e, me); publish({ t: 'reaction', emoji: e, name: me }); }
function floatEmoji(e: string, name: string) {
  reactionSound(e);
...
function publish(o: any) { if (!room) return; try { room.localParticipant.publishData(new TextEncoder().encode(JSON.stringify(o)), { reliable: true }); } catch (e) { console.warn(e); } }

// ---------- panels ----------
export function openPanel(w: string) {
  curPanel = w; $('panel').classList.add('open');
  const people = w === 'people';
  $('pnlTitle').textContent = people ? 'People' : 'In-call messages';
  $('peopleBody').style.display = people ? 'block' : 'none';
  $('chatBody').style.display = people ? 'none' : 'block';
  $('chatBar').style.display = people ? 'none' : 'flex';
  $('dPeople').classList.toggle('accent', people);
  $('dChat').classList.toggle('accent', !people);
  if (people) renderPeople(); else { unread = 0; $('chatCnt').style.display = 'none'; renderChat(); setTimeout(() => $('chatInput').focus(), 40); }
}
export function closePanel() { $('panel').classList.remove('open'); $('dPeople').classList.remove('accent'); $('dChat').classList.remove('accent'); curPanel = null; }
function renderPeople() {
  if (!room) return; const ps = parts(); let h = `<div class="psec">In the room · ${ps.length}</div>`;
  ps.forEach(p => {
    const local = p === room.localParticipant; const mic = p.getTrackPublication(LK.Track.Source.Microphone); const muted = !mic || !mic.track || mic.isMuted;
...
// ---------- chat ----------
export function grow(el: any) { el.style.height = 'auto'; el.style.height = Math.min(el.scrollHeight, 90) + 'px'; }
export function onChatInput() { $('chatSend').disabled = !$('chatInput').value.trim(); }
export function chatKey(e: any) { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); sendChat(); } }
export function sendChat() { const inp = $('chatInput'); const t = inp.value.trim(); if (!t) return; const nm = room?.localParticipant?.name || 'You'; addChat(nm, t, true); publish({ t: 'chat', text: t, name: nm }); inp.value = ''; grow(inp); onChatInput(); }
function addChat(name: string, text: string, mine?: boolean) { chatLog.push({ name, text, ts: clock(), mine }); if (curPanel === 'chat') renderChat(); else { unread++; const b = $('chatCnt'); b.style.display = 'flex'; b.textContent = unread; } }
function renderChat() {
  const box = $('chatMsgs'); if (!chatLog.length) { box.innerHTML = '<div class="chat-empty">Messages are visible to everyone in the room and disappear when the meeting ends.</div>'; return; }
...
// ---------- share / leave ----------
export function copyLink() { navigator.clipboard?.writeText(shareUrl).then(() => toast('Invite link copied')).catch(() => toast('Code: ' + roomId)); }
export function copyShare() { const i = $('shareLink'); i.select(); navigator.clipboard?.writeText(i.value).then(() => toast('Link copied — share it!')).catch(() => {}); }
export function closeShare() { $('shareModal').classList.remove('open'); }
export function onLeaveClick(e: any) {
  e.stopPropagation();
  // Host gets a choice (leave vs end for everyone); guests just leave.
  if (myRole === 'host' && livestreamUuid) { $('leaveMenu').classList.toggle('open'); }
  else { leaveMeeting(); }
}
function closeLeaveMenu() { $('leaveMenu').classList.remove('open'); }
export function leaveMeeting() { closeLeaveMenu(); if (room) room.disconnect(); }
...
function cleanup() {
  // ----- AUDIO FIX (d): clean teardown -----
  // After the room disconnects, explicitly stop every LOCAL mediaStreamTrack so
  // the mic/camera are actually released and the NEXT meeting can re-acquire
  // them (a mic left held reads as "dead" on the following call).
  if (room && room.localParticipant) {
    ...
    } catch (e) { console.warn('local media teardown failed', e); }
  }
  tileEls.forEach(el => { el.querySelectorAll('video,audio').forEach((m: any) => { try { m.srcObject = null; } catch {} }); el.remove(); }); tileEls.clear(); if (stripEl) { stripEl.remove(); stripEl = null; }
  ...
  $('grid').innerHTML = ''; closePanel(); closeLeaveMenu(); $('shareModal').classList.remove('open');
  updateAudioGate();
  ['btnGuestJoin', 'btnStart'].forEach(id => { const b = $(id); if (b) b.disabled = false; });
  $('btnGuestJoin').innerHTML = '<span class="material-symbols-rounded">login</span> Join now';
  $('btnStart').innerHTML = '<span class="material-symbols-rounded">rocket_launch</span> Start meeting';
  show('landing'); refreshRejoinCard(); toast('You left the meeting');
}

// ---------- startup ----------
// The original ran these at the bottom of app.js on load. In the React port they
// run once, after the components have mounted the identical markup, so all the
...
export function initEngine() {
  ...
  // Re-apply the saved/system theme (the pre-paint bootstrap in index.html set it
  // first; this keeps applyTheme() the single source of truth once loaded).
  ...
  // Expose the handlers referenced by dynamically-built innerHTML (the host tile
  // menu uses inline onclick="setPin(...)" / moderate(...) / closeTileMenu()).
  Object.assign(window as any, { setPin, moderate, closeTileMenu });

  // ----- AUDIO FIX (b): background/restore -----
  // Chrome suspends WebRTC audio when the tab is hidden; on return, re-arm audio
  // playback, resume the reaction AudioContext, and re-run the grid render so
  // every <audio>/<video> re-attaches.
  document.addEventListener('visibilitychange', () => {
    if (document.visibilityState === 'visible' && room && room.state === LK.ConnectionState.Connected) {
      room.startAudio().catch(() => {});
      try { _actx && _actx.resume && _actx.resume(); } catch {}
      renderGrid();
      updateAudioGate();
  ...
  document.addEventListener('click', e => {
    const tm = $('tileMenu');
    if (tm && tm.classList.contains('open') && !tm.contains(e.target) && !(e.target as any).closest('.tmore')) closeTileMenu();
    const m = $('leaveMenu'); if (m.classList.contains('open') && !m.contains(e.target) && !(e.target as any).closest('.dbtn.leave')) closeLeaveMenu();
  });
