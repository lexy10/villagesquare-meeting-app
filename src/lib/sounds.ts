...
const REACTION_SOUNDS: Record<string, (ac: AudioContext, o: AudioNode) => void> = {
  ...
  '\u{1F44F}': (ac, o) => { [0, 0.11, 0.21, 0.3].forEach((t, i) => noise(ac, o, { at: t, dur: 0.075, peak: i ? 2.0 : 2.7, f0: 1600 + Math.random() * 900, q: 0.7 })); }, // applause
...
// Public-domain, loudness-matched to the synthesized cues (sounds/CREDITS.txt).
const REACTION_SAMPLES: Record<string, string> = { '\u{1F44F}': 'sounds/clap.mp3', '\u{1F602}': 'sounds/laugh.mp3' };
...
export function warmReactionSamples() {
  try { const ac = ctx(); Object.values(REACTION_SAMPLES).forEach(u => loadSample(ac, u).catch(() => {})); } catch { /* ignore */ }
}
